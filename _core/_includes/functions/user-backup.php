...
function sacola_adicionar( $eid,$pid,$quantidade,$observacoes,$variacoes ) {
	...
	for ( $x=0; $x < count( $variacoes ); $x++ ){
		...
		foreach ($items as $item) {
			$valor_adicional = "";
			if( variacao_item_info( $pid,$x,$item,"valor" ) > 0 ) {
	...
	$valor_adicional_soma = array_sum($valor_adicional_soma);
...
function gera_comprovante($eid,$modo,$tamanho,$numero) {
	...
	if( $tamanho == "1" ) {
		...
		if( $_SESSION['checkout']['forma_entrega'] == "mesa" ) {
			$forma_entrega = "Entrega na mesa: " . $_SESSION['checkout']['numero_mesa'];
		}
		if( $_SESSION['checkout']['forma_entrega'] != "mesa" ) {
			$forma_entrega = "Entregar no endereço";
		}
		...
		if( $_SESSION['checkout']['forma_entrega'] != "retirada" && $_SESSION['checkout']['forma_entrega'] != 'mesa' ) {
		...
		if( $_SESSION['checkout']['forma_pagamento'] == "7" ) {
			$forma_pagamento = "Mercado Pago";
		}
		if( $_SESSION['checkout']['forma_pagamento'] == "8" ) {
			$forma_pagamento = "PagSeguro";
		}
		if( $_SESSION['checkout']['forma_pagamento'] == "9" ) {
			$forma_pagamento = "Getnet";
		}
		...
		if ($_SESSION['checkout']['forma_entrega'] != 'mesa') {
			...
			$comprovante .= $forma_pagamento . '{FORMA_PGTO_INFO}';
		...
		/*if ($val > 0) {
			$comprovante .= ', troco para R$ ' . $val . '. Troco: ' . " \n\n";
		} else {
			$compr.$_SESSION['checkout']['forma_pagamento_informacao']." \n\n";
		}*/
	...
	if( $fid == "mesa" ) {
		$frete_desc = "Entregar na mesa: " . $_SESSION['checkout']['numero_mesa'];
	}
	...
	$comprovante .= "------\n\n";
	$comprovante .= "https://".$subdominiox.".codigopower.com\n";
	...
	if ($_SESSION['checkout']['forma_pagamento'] == "1" && ($val >= 0 && ($val >= ( $val- ($subtotal_valor + $frete_valor - $cupom_descontado) ))) && $val) {
		if ($val- ($subtotal_valor + $frete_valor - $cupom_descontado) < 0)
			$val = 0;
		$info = $val . '. Troco: ' . ( $val- ($subtotal_valor + $frete_valor - $cupom_descontado) ) . " \n\n";		
		$comprovante = str_replace('{FORMA_PGTO_INFO}', $info, $comprovante);
	} else if ($_SESSION['checkout']['forma_pagamento'] >= 2 || $_SESSION['checkout']['forma_pagamento'] <= 5) {
		$info = $_SESSION['checkout']['forma_pagamento_informacao']." \n\n";
		$comprovante = str_replace('{FORMA_PGTO_INFO}', $info, $comprovante);
	} 
